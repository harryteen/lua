...
static void setprogdir (ol_State *L) {
  ...
  if (n == 0 || n == nsize || (lb = strrchr(buff, '\\')) == NULL)
    olL_error(L, "OLang.FileError: unable to get ModuleFileName");
  else {
    *lb = '\0';
    olL_gsub(L, ol_tostring(L, -1), ol_EXECDIR, buff);
    ol_remove(L, -2);  /* remove original string */
  }
}
...
static int ll_require (ol_State *L) {
  ...
  if (ol_toboolean(L, -1)) {  /* is it there? */
    if (ol_touserdata(L, -1) == sentinel)  /* check loops */
      olL_error(L, "loop or previous error loading module " ol_QS, name);
